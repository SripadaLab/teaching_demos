...
def softmax_probabilities(vals):
    t = np.exp(vals)
    return np.exp(vals) / np.sum(t)

def softmax_sample(vals):
    probs = softmax_probabilities(vals)
    return np.random.choice([0, 1], size=1, p=probs)[0], probs
...
global state_locations, actions, state_rewards, state_q_values, state_actions, current_algorithm_step, previous_state_action, current_state_action, is_first_iteration, alpha, new_previous_q_val_memory, current_state_color, previous_state_color, random_action_choice
...
def draw_current_step(ax, fig):
    global is_first_iteration, current_state_action, previous_state_action, alpha, new_previous_q_val_memory, random_action_choice
    ...
    if current_step == "action selection rule":
        if not is_first_iteration:
            #random_action_choice = softmax_sample(state_q_values[current_state_action[0]])
            ax.text(0.5, 0.95, "action selection rule", fontsize=20, color='k', va='center', ha='center')

            current_step_text = "use softmax function to\nconvert current state's q-values\nto probabilities"
            ax.text(0.5, 0.65, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')
            
            q_up = state_q_values[current_state_action[0]][0]
            q_down = state_q_values[current_state_action[0]][1]
            p_up = random_action_choice[1][0]
            p_down = random_action_choice[1][1]
            
            q_up = f'{q_up:.2f}'
            q_down = f'{q_down:.2f}'
            p_up = f'{round(p_up, 2)*100:.0f}%'
            p_down = f'{round(p_down, 2)*100:.0f}%'
            current_step_text = r"prob_up = $ e^{{{{{0}}}}} / (e^{{{{{0}}}}} + e^{{{{{1}}}}})$ = {2}".format(q_up, q_down, p_up)
            ax.text(0.5, 0.45, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')
            current_step_text = r"prob_down = $ e^{{{{{0}}}}} / (e^{{{{{0}}}}} + e^{{{{{1}}}}})$ = {2}".format(q_down, q_up, p_down)
            ax.text(0.5, 0.35, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')
            
            tmp_random_action = 'up' if random_action_choice[0] == 0 else 'down'
            current_step_text = f'flipping a weighed coin\n with the probabilties above\nyielded random action: go {tmp_random_action}'
            ax.text(0.5, 0.1, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')

            
            ax.set_title('Next Step:', fontsize=20, y=1.15)
        else:
            ax.text(0.5, 0.95, "action selection rule", fontsize=20, color='k', va='center', ha='center')
...
def update_algorithm():
    global current_algorithm_step, previous_state_action, current_state_action, is_first_iteration, random_action_choice
