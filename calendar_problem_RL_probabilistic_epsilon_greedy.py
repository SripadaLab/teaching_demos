...
global state_locations, actions, state_rewards, state_q_values, state_actions, current_algorithm_step, previous_state_action, current_state_action, is_first_iteration, alpha, new_previous_q_val_memory, current_state_color, previous_state_color, random_sample
global epsilon
...
def reset():
    ...
    epsilon = 0.33
    random_sample = round(np.random.uniform(0, 1), 2)
    random_action_choice = np.argmax(state_q_values[0]) if random_sample > epsilon else np.random.choice([0, 1])
    current_state_action = [0, random_action_choice]
...
def draw_current_step(ax, fig):
    global is_first_iteration, current_state_action, previous_state_action, alpha, new_previous_q_val_memory, random_action_choice, random_sample
    ...
    if current_step == "start":
        if is_first_iteration:
            ...
            random_sample = round(np.random.uniform(0, 1), 2)
            random_action_choice = np.argmax(state_q_values[0]) if random_sample > epsilon else np.random.choice([0, 1])  #softmax_sample(state_q_values[0])  #softmax_sample([0, 0])
            current_state_action = [0, random_action_choice]
            previous_state_action = None
            is_first_iteration = True
    ...
    if current_step == "action selection rule":
        ax.text(0.5, 1.1, r"action selection rule (with $\epsilon$ = "+f'{epsilon})', fontsize=20, color='k', va='center', ha='center')

        current_step_text = 'choose action in the\ncurrent state with\nmaximal Q value'
        ax.text(-0.2, 0.85, current_step_text, ha='left', va='center', 
                fontsize=20, color='k')

        current_step_text = r'with probability 1-$\epsilon$,'
        ax.text(0.575, 0.85, current_step_text, ha='left', va='center', 
                fontsize=20, color='k')

        current_step_text = 'choose a random\naction in the\ncurrent state'
        ax.text(-0.2, 0.5, current_step_text, ha='left', va='center', 
                fontsize=20, color='k')

        current_step_text = r'with probability $\epsilon$.'
        ax.text(0.575, 0.5, current_step_text, ha='left', va='center', 
                fontsize=20, color='k')

        l = lines.Line2D([-0.25, -0.25], [1, 0.35], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)
        l = lines.Line2D([-0.25, -0.2], [1, 1], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)
        l = lines.Line2D([-0.25, -0.2], [0.35, 0.35], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)
        l = lines.Line2D([1.3, 1.3], [1, 0.35], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)
        l = lines.Line2D([1.3, 1.25], [1, 1], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)
        l = lines.Line2D([1.3, 1.25], [0.35, 0.35], clip_on=False, linewidth=2, color='k')
        ax.add_line(l)

        current_step_text = f'we draw a random number between\n0 and 1... this time we drew {random_sample}'
        ax.text(0.5, 0.2, current_step_text, ha='center', va='center', 
                fontsize=20, color='k')

        if random_sample <= epsilon:
            extra_text = 'go up' if current_state_action[1] == 0 else 'go down'
            current_step_text = f'since {random_sample} is less than or equal to {epsilon} ' + r'($\epsilon$)' + '\nwe choose a random action in current\nstate by flipping a coin... ' + extra_text
            ax.text(0.5, -0.05, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')
        else:
            current_step_text = f'since {random_sample} is greater than {epsilon} '+ r'($\epsilon$)' + '\nwe choose the action with maximal Q-value\nin the current state'
            ax.text(0.5, -0.05, current_step_text, ha='center', va='center', 
                    fontsize=20, color='k')



        ax.set_title('Next Step:', fontsize=20, y=1.25)
    ...
    if current_step == "update previous/current state-actions":
        if is_first_iteration:
            ...
            ax.text(0.5, 0.8, 'update previous state-action Q value', ha='center', va='center', 
                    fontsize=18, color='k')
            ax.text(0.5, 0.3, 'follow current Q value action\nfrom current state to\nget the next "current state",\nand set the next\n"previous state-action"\nto current state-action', ha='center', va='center', 
                    fontsize=18, color='k')
            ax.set_title('Next Step:', fontsize=20, y=1)
...
def update_algorithm():
    global current_algorithm_step, previous_state_action, current_state_action, is_first_iteration, random_action_choice, random_sample, epsilon
    ...
    if current_algorithm_step == 5:
        if previous_state_action is None:
            ...
            random_sample = round(np.random.uniform(0, 1), 2)
            random_action_choice = np.argmax(state_q_values[current_state]) if random_sample > epsilon else np.random.choice([0, 1])
            current_action = random_action_choice
